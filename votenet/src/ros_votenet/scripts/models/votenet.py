...
class VoteNet(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        self.num_class = num_class                          # ST: 10
        self.num_heading_bin = num_heading_bin              # ST: 12
        self.num_size_cluster = num_size_cluster            # ST: 10
        self.mean_size_arr = mean_size_arr
        assert(mean_size_arr.shape[0] == self.num_size_cluster)
        self.input_feature_dim = input_feature_dim          # ST: 1
        self.num_proposal = num_proposal                    # ST: 256
    ...
    def forward(self, inputs):
        ...
        # --------- HOUGH VOTING ---------
        xyz = end_points['fp2_xyz']                         #ST: [1, 1024, 3]
        features = end_points['fp2_features']               #ST: [1, 256, 1024]
        end_points['seed_inds'] = end_points['fp2_inds']    #ST: [1, 1024]
        end_points['seed_xyz'] = xyz
        end_points['seed_features'] = features
        
        xyz, features = self.vgen(xyz, features)            # These are actual votes from the seed points
        print('------- Voting Module --------')

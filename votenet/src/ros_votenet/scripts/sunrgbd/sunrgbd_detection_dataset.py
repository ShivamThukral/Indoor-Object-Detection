    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            if np.random.random() > 0.5:
                ...
                bboxes[:,0] = -1 * bboxes[:,0] # S: flip the x coordinate of the BB
                bboxes[:,6] = np.pi - bboxes[:,6]  # S: flip the heading angle of the BB
                point_votes[:,[1,4,7]] = -1 * point_votes[:,[1,4,7]] # S: flip the X coordinate of the vote as well
            ...
            point_votes_end[:,1:4] = np.dot(point_cloud[:,0:3] + point_votes[:,1:4], np.transpose(rot_mat))  # S: transform the first votes
...
if __name__=='__main__':
    ...
    sample = d[200] # S : Not working for all the objects
